...
public abstract class FormCargoBase extends Dialogo {
...
protected Cargo retorno;




	public FormCargoBase(String title, Cargo retorno) {
	...
	protected boolean validation(){
		...
		Mensagens.msgDeErro("Informe um código para o cargo.");
		Comuns.textFieldErroMode(this.tf_codigo);
		return false;	
		}
			
			
		if(!Comuns.codigoPermitido(new DAO<Cargo>(Cargo.class), this.tf_codigo.getText(), this.retorno!=null?this.retorno.getId_cargo():0)){
				
		Mensagens.msgDeErro("O código informado já está sendo usado por outro cargo.");
		Comuns.textFieldErroMode(this.tf_codigo);
		return false;	
		}
		...
		Mensagens.msgDeErro("Informe a descrição deste cargo.");
